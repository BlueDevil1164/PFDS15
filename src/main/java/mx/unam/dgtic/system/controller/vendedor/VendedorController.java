package mx.unam.dgtic.system.controller.vendedor;

import mx.unam.dgtic.system.exception.VendedorNoExisteExepcion;
import mx.unam.dgtic.system.model.Vendedor;
import mx.unam.dgtic.system.servicio.vendedor.IVendedorService;
...
public class VendedorController {
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<Void> deleteVendedor(@PathVariable("id") int id) throws  VendedorNoExisteExepcion {
    ...
    @PatchMapping("/{id}")
    public ResponseEntity<Vendedor> updatePartialVendedor(@RequestBody Vendedor vendedor, @PathVariable("id") int id) throws VendedorNoExisteExepcion {
        Optional<Vendedor> vendedorDB = vendedorService.getVendedorById(id);
        if (vendedorDB.isPresent()) {
